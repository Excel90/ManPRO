...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  if (isset($_POST['id_course'])) {
    $sql = "SELECT * FROM class JOIN course ON course.id_course = class.id_course WHERE class.id_class = ?";
    ...
    $sql = "SELECT * FROM `upload` INNER JOIN class ON class.id_class = upload.id_class;";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $fetch3 = $stmt->fetch();
    $_SESSION['id_class'] = $fetch['id_class']; 
  }
} else {
...
<body>
  <div class="container-fluid" style="background: #5a47ab; width: 100%; left:0; position:fixed; width:100vw; height:100vh;">
    ...
    <div id="payment" class="<?php echo $payment ?>">
      <div class="d-flex justify-content-center">
          ...
          <div class="row text-center">
            <div class="col p-3" style="background: white;">
              <span style="border-style: none; background: white;">
                <h2 style="color: #5a47ab;">Payment</h2>
              </span>
            </div>
          </div>
          <div class="row text-center" style="margin-top: 3em; color:white;">
            <h4>Class: <?php echo $fetch['class_name']?></h4>
            <h4></h4>
            <h3>Proceed your payment to</h3>
            <h2 style="margin-top: 0.5em">BCA 1020512241</h2>
            <h4 style="margin-top: 2em">Your Total</h4>
            <h2 style="margin-top: 0.5em">IDR <?php echo number_format($fetch['price'])?>,000</h2>
            <form action="../Manage/payment.php" method="POST" style="margin-top: 2em;" enctype="multipart/form-data">
              <input type="file" accept="image/*" id="berkas" name="berkas" style="width: 10em;height: 3em;">
